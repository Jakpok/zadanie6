...
public class HardwareResources {
	...
	public Response get(@PathParam("id") int id) {
		Hardware hardware = em.createNamedQuery("hardware.id", Hardware.class).setParameter("hardwareID", id).getSingleResult();
		if(hardware==null) {
	...
	public Response update(@PathParam("id") int id, Hardware h) {
		...
		hardware.setName(h.getName());
		hardware.setCategory(h.getCategory());
		hardware.setPriceMin(h.getPriceMin());
		hardware.setPriceMax(h.getPriceMax());
		em.persist(hardware);
		return Response.ok("success").build();
	...
	public Response getByName(@PathParam("name") String name){
		Hardware hardware = em.createNamedQuery("hardware.name", Hardware.class).setParameter("hardwareName", name).getSingleResult();
		if(hardware==null) {
			return Response.status(404).build();
		}
		return Response.ok(hardware).build();
	...
	public List<Hardware> getGraphics() {
		return em.createNamedQuery("hardware.category", Hardware.class).
				setParameter("hardwareCategory", HardwareType.GRAPHICS_CARD)
				.getResultList();
	}
	...
	public List<Hardware> getMainboards() {
		return em.createNamedQuery("hardware.category", Hardware.class).
				setParameter("hardwareCategory", HardwareType.MOTHERBOARD).getResultList();
	}
	...
	public List<Hardware> getDiscs() {
		return em.createNamedQuery("hardware.category", Hardware.class).
				setParameter("hardwareCategory", HardwareType.HDD).getResultList();
	}
	...
	public List<Hardware> getRam() {
		return em.createNamedQuery("hardware.category", Hardware.class).
				setParameter("hardwareCategory", HardwareType.RAM).getResultList();
	}
	...
	public Response setComment(@PathParam("id") int id, Comment comment) {
		Hardware hardware = em.createNamedQuery("hardware.id", Hardware.class)
							.setParameter("hardwareID", id).getSingleResult();
		if(hardware==null) {
			return Response.status(404).build();
		}
		hardware.getComments().add(comment);
	...
	public List<Comment> getComment(@PathParam("id") int hardwareId) {
		Hardware hardware = em.createNamedQuery("hardware.id", Hardware.class)
							.setParameter("hardwareID", hardwareId).getSingleResult();
		if(hardware==null) {
			return null;
		}
		return hardware.getComments();
	}
